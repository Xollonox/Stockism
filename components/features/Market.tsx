...
import { PremiumCard, StatusBadge, GradientBar, AnimatedBg } from '../premium';
...
export const Market: React.FC<MarketProps> = React.memo(({ market, search, setSearch, onTrade, onCardClick, settings, frozenIds, recentSearches, onClearRecentSearches }) => {
  ...
  return (
    <div className="space-y-8">
       ...
       <PremiumCard glow className="!p-0 overflow-hidden">
         <div className="relative overflow-hidden bg-gradient-to-r from-brand/10 to-transparent border-l-4 border-brand p-8">
           <AnimatedBg />
           <div className="relative z-10 flex flex-col md:flex-row justify-between items-end gap-6">
               ...
               <div className="flex items-center gap-3 animate-fade-in-up">
                   <StatusBadge status="active" label="Active Protocol" />
                   <StatusBadge status="online" label={`${filteredMarket.length} Nodes`} />
               </div>
           </div>
           ...
           <GradientBar className="absolute bottom-0" />
         </div>
       </PremiumCard>
